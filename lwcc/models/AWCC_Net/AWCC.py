...
from lwcc.models.AWCC_Net.transformer_decoder import Block_dec
...
class AWCCNet(nn.Module):
    def __init__(self, use_pe=False, output_dim=1):
        ...
        self.weather_encoder = WeatherEncoder(
            n_downsample=4,
            dim=64,
            style_dim=n_prototypes,
            norm="none",
            activ="relu",
            pad_type="reflect",
            input_dim=512,
        )
        self.weather_bank = nn.Parameter(
            torch.randn((n_prototypes, n_tokens, d_model)), requires_grad=True
        )
        ...
        self.encoder = nn.TransformerEncoder(
            encoder_layer=encoder_layer, num_layers=num_layers, norm=None
        )
        ...
        # --- load pretrain ---
        vgg_pretrained_checkpoint = torch.hub.load_state_dict_from_url(
            model_urls["vgg19"]
        )
        torch.nn.modules.utils.consume_prefix_in_state_dict_if_present(
            vgg_pretrained_checkpoint, prefix="features."
        )
    ...
    def forward(self, x):
        ...
        self.weight_vectors = self.weather_encoder(
            vgg_feats, return_logit=True
        )
        self.weight_vectors = F.softmax(self.weight_vectors, dim=-1)

        # transformer encoder
        bs, c, h, w = vgg_feats.shape
        vgg_feats = vgg_feats.flatten(2).permute(
            0, 2, 1
        )  # [b, h*w, c] = [b, n_tokens, dim]
        ...
        # transformer decoder
        style_q = torch.einsum(
            "bs, snc -> bnc", self.weight_vectors, self.weather_bank
        )
        ...
        x = F.interpolate(
            x, size=(rh, rw), mode="bilinear", align_corners=True
        )
...
class PositionalEncoding2d(nn.Module):
    ...
    def forward(self, x: Tensor, h, w, nbc=True) -> Tensor:
        ...
        if h != 16 or w != 16:
            pe = F.interpolate(
                self.pe, size=(h, w), mode="bicubic", align_corners=False
            )
        else:
            pe = self.pe
...
class WeatherEncoder(nn.Module):
    def __init__(
        self, n_downsample, dim, style_dim, norm, activ, pad_type, input_dim=3
    ):
        super().__init__()
        layers = []
        layers += [
        ...
        layers += [nn.AdaptiveAvgPool2d(1)]  # global average pooling
